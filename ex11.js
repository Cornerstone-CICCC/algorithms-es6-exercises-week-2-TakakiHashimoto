/*
There's a new attraction at this year's Codeville festival. The organizers have decided to bring in several inflatable attractions, 
but they have no clue how to much blow them up. Each attraction needs to be pumped to a precise volume to achieve maximum festival fun!
...
const totalVolume = function (solids) {
  ...
  for (let obj of solids){
    console.log(obj)
    if (obj.type === "sphere"){
      const sphere = sphereVolume(obj.radius);
      total += sphere;
    } else if (obj.type === "cone"){ 
      const cone = coneVolume(obj.radius, obj.height)
      total += cone;
    } else if (obj.type === "prism"){
      const prism = prismVolume(obj.height, obj.width, obj.depth);
      total += prism
    }
  }
  console.log(total)
