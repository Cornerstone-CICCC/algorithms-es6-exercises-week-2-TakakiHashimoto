/*
Welcome back to Codeville! The citizens of Codeville seem pleased with all the upgrades your administration has been making to the local 
infrastructure, and they want more! The parking lot in the Codeville Devtropolis Shopping Mall needs an upgrade, 
...
Note
Note: There may be multiple available spots for a particular vehicle. It does not matter which spot your function chooses, 
as long as the spot is available. And if there are no available spots, remember to return false.
*/
...
const whereCanIPark = function (spots, vehicle) {
  ...
  const parkingSpot = [];
  const vehicleAvailable = parkingObj[vehicle];
  // console.log(vehicleAvailable)
  let breakPoint = 0;
  spots.forEach(function(array, yIndex){ // for each array
    // console.log(array)
    for (const spot of vehicleAvailable){ // taking out the each avalibale spot for the vehicle
      // console.log(array.includes(spot))
      if (array.includes(spot)){ // if that spot is included in the current array,
        for (xIndex in array){
          if (spot === array[xIndex]){
            parkingSpot.push(parseInt(xIndex), yIndex);
            breakPoint ++;
            break;
          }
        }
